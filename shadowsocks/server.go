...
func handleConnection(conn *Conn, auth bool) {
	var host string
	...
	log.Println(fmt.Sprintf("pipelining tcp transmission %s <-> %s <-> %s", conn.RemoteAddr(), conn.LocalAddr(), host))

	if ota {
		go PipeThenCloseOta(conn, remote)
	} else {
...
func (pm *PasswdManager) addUDP(port, password string, listener *net.UDPConn, isClosed bool) {
	pm.Lock()
	pm.udpListener[port] = &UDPListener{password, listener, isClosed}
	pm.Unlock()
}
...
func (pm *PasswdManager) updatePortPasswd(port, password string, auth bool, mgr *Manager) {
	pl, ok := pm.get(port)
	if ok {
		if pl.password == password {
			return
		}
		log.Printf("closing tcp port %s to update password\n", port)
		pl.listener.Close()
	}
	// run will add the new port listener to passwdManager.
	// So there maybe concurrent access to passwdManager and we need lock to protect it.
	go run(port, password, auth, mgr)
	...
	if udp {
		...
		if ok {
			...
			if err := pl.listener.Close(); err != nil {
				log.Printf("error closing udp listener port %s: %v\n", port, err)
			}
			// wait udp event loop exits
			pl.isClosed = true
			time.Sleep(1 * time.Second)
		}
		go runUDP(port, password, auth, mgr)
	}
}

var passwdManager = PasswdManager{portListener: map[string]*PortListener{}, udpListener: map[string]*UDPListener{}}

func updatePasswd(mgr *Manager) {
	...
	for port, passwd := range config.PortPassword {
		passwdManager.updatePortPasswd(port, passwd, config.Auth, mgr)
...
func run(port, password string, auth bool, mgr *Manager) {
	...
	for {
		...
		if cipher == nil {
			...
			cipher, err = NewCipher(config.Method, password)
			if err != nil {
		...
		go handleConnection(NewConn(conn, cipher.Copy(), mgr), auth)
	}
}

func runUDP(port, password string, auth bool, mgr *Manager) {
	var cipher *Cipher
	...
	defer conn.Close()
	passwdManager.addUDP(port, password, conn, false)
	log.Printf("add udp port %s\n", port)
	cipher, err = NewCipher(config.Method, password)
	if err != nil {
	...
	SecurePacketConn := NewSecurePacketConn(conn, cipher.Copy(), auth, mgr)
	defer SecurePacketConn.Close()
	for {
		pl, ok := passwdManager.getUDP(port)
		if !ok || pl.isClosed {
			break
		}
		if err := ReadAndHandleUDPReq(SecurePacketConn); err != nil {
			debug.Println("ReadAndHandleUDPReq", err)
		}
	}
}
...
func main() {
	...
	if err = CheckCipherMethod(config.Method); err != nil {
	...
	mgr := NewManager(&passwdManager)
	mgr.Run()

	for port, password := range config.PortPassword {
		go run(port, password, config.Auth, mgr)
		if udp {
			go runUDP(port, password, config.Auth, mgr)
		}
	}
